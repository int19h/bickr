...
const avatarProvider = createAvatarProvider(avatarProviderRuntime);
...
const avatarPromptSettingsRuntime = {
...
async function handleAgentRuntimeRequestExclusive(
	...
	try {
		...
		if (userAvatarPromptMatch && request.method === 'POST') {
			...
			const input = parseAvatarPromptRequest(await readOptionalJsonBody(request));
			if (request.headers.get('accept')?.includes('text/event-stream')) {
				return streamAvatarPrompt(env, { kind: 'user', userId }, input, avatarProvider, avatarPromptSettingsRuntime, request.signal);
			}
			const prompt = await prefillAvatarPrompt(env, { kind: 'user', userId }, input, avatarProvider, avatarPromptSettingsRuntime);
		...
		if (userAvatarGenerateMatch && request.method === 'POST') {
			...
			const input = parseAvatarGenerationRequest(await readJsonBody(request));
			if (request.headers.get('accept')?.includes('text/event-stream')) {
				return streamAvatarGeneration(env, { kind: 'user', userId }, input, avatarProvider, request.signal);
			}
			const candidate = await generateAvatar(env, { kind: 'user', userId }, input, avatarProvider);
		...
		if (avatarPromptMatch && request.method === 'POST') {
			...
			const input = parseAvatarPromptRequest(await readOptionalJsonBody(request));
			if (request.headers.get('accept')?.includes('text/event-stream')) {
				return streamAvatarPrompt(env, { kind: 'bot', userId, botId }, input, avatarProvider, avatarPromptSettingsRuntime, request.signal);
			}
			const prompt = await prefillAvatarPrompt(env, { kind: 'bot', userId, botId }, input, avatarProvider, avatarPromptSettingsRuntime);
			return ok({ prompt, coordinator: objectId });
		}
		...
		if (avatarGenerateMatch && request.method === 'POST') {
			...
			const input = parseAvatarGenerationRequest(await readJsonBody(request));
			if (request.headers.get('accept')?.includes('text/event-stream')) {
				return streamAvatarGeneration(env, { kind: 'bot', userId, botId }, input, avatarProvider, request.signal);
			}
			const candidate = await generateAvatar(env, { kind: 'bot', userId, botId }, input, avatarProvider);
			return ok({ candidate, coordinator: objectId });
		}
		...
		if (avatarApplyMatch && request.method === 'POST') {
			...
			let bot = await applyGeneratedAvatarForBot(env, userId, botId, parseAvatarCandidateValue(body.candidate));
			if (settingsInput?.inferenceSettings !== undefined) {
		...
		if (worldAvatarPromptMatch && request.method === 'POST') {
			...
			const input = parseAvatarPromptRequest(await readOptionalJsonBody(request));
			if (request.headers.get('accept')?.includes('text/event-stream')) {
				return streamAvatarPrompt(env, { kind: 'world', userId, worldHandle }, input, avatarProvider, avatarPromptSettingsRuntime, request.signal);
			}
			const prompt = await prefillAvatarPrompt(env, { kind: 'world', userId, worldHandle }, input, avatarProvider, avatarPromptSettingsRuntime);
			return ok({ prompt, coordinator: objectId });
		}
		...
		if (worldAvatarPromptSettingsMatch && request.method === 'GET') {
			...
			const settings = await resolvedWorldAvatarPromptSettings(env, { kind: 'world', userId, worldHandle }, avatarPromptSettingsRuntime);
		...
		if (worldAvatarGenerateMatch && request.method === 'POST') {
			...
			const input = parseAvatarGenerationRequest(await readJsonBody(request));
			if (request.headers.get('accept')?.includes('text/event-stream')) {
				return streamAvatarGeneration(env, { kind: 'world', userId, worldHandle }, input, avatarProvider, request.signal);
			}
			const candidate = await generateAvatar(env, { kind: 'world', userId, worldHandle }, input, avatarProvider);
			return ok({ candidate, coordinator: objectId });
		}
		...
		if (worldAvatarApplyMatch && request.method === 'POST') {
			...
			let world = await applyGeneratedAvatarForWorld(env, userId, worldHandle, parseAvatarCandidateValue(body.candidate));
