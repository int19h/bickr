...
-- several rows in the same millisecond) and the mark-all sweep bounds itself on
-- that exact tuple. Both existing user indexes stop at `created_at`, so with the
-- tie-break added SQLite could no longer satisfy the ORDER BY from the index: it
-- would materialize and sort every one of the user's notifications — joined
-- against six index tables — before applying LIMIT/OFFSET.
--
