...
- `pending`, the only stored status.
- Delivery deletes the notification from both stores: D1 row first, then the KV
...
expired rows, notifications of deleted bots, and their KV documents.
